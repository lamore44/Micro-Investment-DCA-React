...
import {
  ...
  TouchableOpacity, TextInput as RNInput,
  ActivityIndicator,
} from 'react-native';
...
import { ASSETS, Asset, Frequency, QUICK_PRESETS, fmtUSD } from '../../data/mockData';
...
function ChipPicker<T extends string | number>({
  items, selected, onSelect, labelOf, fullWidth = false,
}: {
  items: readonly T[];
  selected: T | null;
  onSelect: (v: T) => void;
...
export const BuilderScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const applyPreset = useCallback((p: typeof QUICK_PRESETS[0]) => {
    setAsset(p.asset); setAmount(p.amount as AmountPreset);
    setCustomAmt('');  setFrequency(p.frequency);
    setStartDate(p.startDate); setEndDate(p.endDate);
  }, []);

  const handleRun = async () => {
    const res = await run({ asset, amount: effectiveAmount, frequency, startDate, endDate, mcMonths });
    if (res) navigation.navigate('Backtest', { strategy: res.strategy, chartData: res.chartData, mcData: res.mcData, mcMonths });
  };
      ...
      <ScrollView contentContainerStyle={S.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <Card style={S.section}>
          ...
          <ChipPicker
            ...
            onSelect={v => { setAmount(v); setCustomAmt(''); }}
            labelOf={v => `$${v}`}
          />
          <View style={S.amtRow}>
            ...
            <RNInput
              ...
              value={customAmt}
              onChangeText={t => { setCustomAmt(t); setAmount(null); }}
        ...
        <Card style={S.section}>
          ...
          <View style={S.periodRow}>
            {[
              { l: '1Y', s: '2023-01-01', e: '2024-01-01' },
              { l: '2Y', s: '2022-01-01', e: '2024-01-01' },
              { l: '3Y', s: '2021-01-01', e: '2024-01-01' },
              { l: '5Y', s: '2019-01-01', e: '2024-01-01' },
            ].map(p => {
              const active = startDate === p.s && endDate === p.e;
              return (
                <TouchableOpacity key={p.l} style={[S.periodChip, active && S.periodChipActive]}
                  onPress={() => { setStartDate(p.s); setEndDate(p.e); }}>
                  <Text style={[S.periodText, active && S.periodTextActive]}>{p.l}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
        ...
        <Card style={S.section}>
          ...
          <ChipPicker items={MC_MONTHS_LIST} selected={mcMonths} onSelect={setMcMonths} labelOf={v => `${v}m`} />
        </Card>
        ...
        <View style={S.estCard}>
          ...
          <Text style={S.estSub}>{steps} purchases × ${effectiveAmount}</Text>
        </View>
        ...
        <Card style={S.section}>
          ...
          {QUICK_PRESETS.map((p, i) => (
            <TouchableOpacity
              key={i}
              style={[S.presetRow, i < QUICK_PRESETS.length - 1 && S.presetBorder]}
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={S.presetMeta}>${p.amount}/{p.frequency} · {p.startDate.slice(0,7)} → {p.endDate.slice(0,7)}</Text>
              </View>
        ...
        {/* Run button */}
        <TouchableOpacity style={[S.runBtn, loading && { opacity: 0.7 }]} onPress={handleRun} disabled={loading} activeOpacity={0.85}>
          {loading ? (
...
const S = StyleSheet.create({
  ...
  chip:             { paddingHorizontal: 14, paddingVertical: 9, borderRadius: Radius.sm, borderWidth: 1, borderColor: Colors.border },
  chipFlex:         { flex: 1, alignItems: 'center' },
  chipActive:       { backgroundColor: Colors.purpleDim, borderColor: Colors.purple },
  chipText:         { ...Typography.bodyS, color: Colors.muted, fontWeight: '600', fontSize: 13 },
  chipTextActive:   { color: Colors.purple, fontWeight: '700' },

  amtRow:    { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.bgPrimary, borderWidth: 1, borderColor: Colors.border, borderRadius: Radius.sm, paddingHorizontal: Spacing.md, height: 48, marginTop: Spacing.md },
  ...
  dateBtn:     { borderWidth: 1, borderColor: Colors.border, borderRadius: Radius.sm, paddingVertical: 11, paddingHorizontal: 12, alignItems: 'center' },
  dateBtnText: { color: Colors.textPrimary, fontSize: 13, fontFamily: 'monospace' },

  periodRow:        { flexDirection: 'row', gap: 8, marginTop: Spacing.md },
  periodChip:       { flex: 1, alignItems: 'center', paddingVertical: 7, borderRadius: Radius.xs, borderWidth: 1, borderColor: Colors.border },
  periodChipActive: { backgroundColor: Colors.purpleDim, borderColor: Colors.purple },
  periodText:       { fontSize: 12, color: Colors.muted, fontWeight: '600' },
  periodTextActive: { color: Colors.purple, fontWeight: '700' },

  estCard:   { backgroundColor: Colors.purpleDim, borderWidth: 1, borderColor: Colors.purpleBorder, borderRadius: Radius.md, padding: Spacing.xl, marginBottom: 12 },
  estValue:  { ...Typography.valueBig, color: Colors.violet, fontSize: 30, marginTop: 4 },
  estSub:    { ...Typography.caption, textTransform: 'none', letterSpacing: 0, fontSize: 11, marginTop: 4 },

  presetRow:    { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, gap: 12 },
  presetBorder: { borderBottomWidth: 1, borderBottomColor: Colors.border },
  presetBadge:  { width: 38, height: 38, borderRadius: 8, backgroundColor: Colors.purpleDim, borderWidth: 1, borderColor: Colors.purpleBorder, alignItems: 'center', justifyContent: 'center' },
  presetBadgeText: { color: Colors.purple, fontSize: 9, fontWeight: '800' },
  presetTitle:  { ...Typography.body, fontWeight: '600', marginBottom: 2 },
  presetMeta:   { ...Typography.caption, textTransform: 'none', letterSpacing: 0, fontSize: 11 },
  presetChev:   { color: Colors.muted, fontSize: 22 },

  runBtn:  { backgroundColor: Colors.purple, borderRadius: Radius.md, paddingVertical: 18, alignItems: 'center', marginTop: Spacing.md, shadowColor: Colors.purple, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 12, elevation: 6 },
  runText: { color: Colors.white, fontWeight: '800', fontSize: 16, letterSpacing: -0.3 },
});
